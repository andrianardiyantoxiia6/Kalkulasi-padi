...
with st.sidebar:
    ...
    with st.expander("1. Kadar Air & Cuaca"):
        st.write("**Istilah:** Tingkat kandungan air pada gabah saat dipanen.")
        st.write("🌦️ **Simulasi Kondisi:**")
        st.info("• **Pagi / Hujan:** Gabah basah, berat timbangan lebih berat tapi harga dipotong karena cepat berjamur.\n• **Siang / Panas:** Gabah kering, kadar air ideal, harga jual lebih stabil.")
    ...
    with st.expander("2. Biaya Penjemuran"):
        ...
        st.info("Jika Anda punya 1,2 ton (12 kuintal) gabah, sisa kelebihan atau total kuantitas akan dikalikan tarif jemur (misal: 12 x Rp10.000 = Rp120.000).")
    ...
    with st.expander("3. Derajat Sosoh (Kualitas Beras)"):
        ...
        st.info("• **Kurang Sosoh:** Beras masih ada garis merah/kecoklatan (kulit ari).\n• **Sosoh Sedang:** Bersih standar pasaran lokal.\n• **Sosoh Premium:** Putih bersih mengkilap, harga jual lebih tinggi.")
    
    with st.expander("4. Ongkos Giling & Logistik"):
        st.write("**Istilah:** Biaya jasa pabrik (umumnya sistem bagi hasil beras).")
        st.write("🚚 **Simulasi Opsi:**")
        st.info("• **Pihak Pabrik Jemput:** Pabrik ambil ke rumah, ongkos naik 1%.\n• **Diambil Sendiri:** Ongkos standar 10% tapi Anda keluar biaya bensin/transport pribadi.")
        
    st.divider()
    st.caption("💡 Glosarium ini dirancang untuk memudahkan pemahaman petani dan dokumen pelengkap HAKI/Skripsi.")

# --- 1. INPUT DATA UTAMA & Kondisi Panen ---
...
st.subheader("Pengaruh Kadar Air Berdasarkan Cuaca/Waktu")
penyesuaian_kadar_air = st.number_input("Penyesuaian Harga karena Kadar Air (Rp/kg)", value=-300, step=50, help="Masukkan angka negatif jika gabah basah karena hujan/pagi.")
harga_gabah_final = harga_gabah_dasar + penyesuaian_kadar_air
st.info(f"Harga Gabah Final Setelah Penyesuaian: **Rp {harga_gabah_final:,} /kg**")
...
# --- 3. DERAJAT SOSOH & HARGA PASAR ---
st.header("3. Kualitas Giling & Harga Pasar")

pilihan_sosoh = st.selectbox(
    "Bagaimana kondisi fisik hasil gilingan beras?",
    [
...
st.caption(f"Estimasi Harga Beras Setelah Disesuaikan Kualitas: Rp {harga_beras_final:,} /kg")
