...
# --- 5. HASIL ANALISIS & RINCIAN TRANSPARAN ---
st.header("5. Hasil Analisis & Rincian Transparan")
if st.button("Hitung dan Tampilkan Analisis Lengkap"):
    
    # Kalkulasi Jual Gabah
    pendapatan_jual_gabah = berat_gabah_kg * harga_gabah_final
    
    # Kalkulasi Olah Beras
    ...
    if selisih > 0:
        st.success(f"**Rekomendasi Keputusan:** Lebih menguntungkan **DIOLAH MENJADI BERAS** dengan selisih keuntungan sebesar **Rp {selisih:,.0f}**.")
    else:
        st.warning(f"**Rekomendasi Keputusan:** Lebih menguntungkan **DIJUAL GABAH LANGSUNG** karena selisih kerugian/penurunan jika diolah mencapai **Rp {abs(selisih):,.0f}**.")
        
    # RINCIAN TRANSPARAN (BREAKDOWN)
    st.subheader("📋 Rincian Parameter & Kalkulasi Sistem")
    
    with st.expander("🔍 Lihat Rincian Hitungan Opsi Jual Gabah Langsung", expanded=True):
        st.write(f"• **Total Berat Gabah:** {berat_gabah_kg:,.0f} kg ({berat_gabah_ton} Ton)")
        st.write(f"• **Harga Dasar Gabah:** Rp {harga_gabah_dasar:,} /kg")
        st.write(f"• **Akumulasi Penyesuaian (Waktu/Cuaca/Kondisi):** Rp {penyesuaian_kadar_air:,} /kg")
        st.write(f"• **Harga Jual Gabah Final:** Rp {harga_gabah_final:,} /kg")
        st.markdown(f"**Rumus:** {berat_gabah_kg:,.0f} kg × Rp {harga_gabah_final:,} = **Rp {pendapatan_jual_gabah:,.0f}**")

    with st.expander("🔍 Lihat Rincian Hitungan Opsi Olah Menjadi Beras", expanded=True):
        st.write(f"• **Berat Beras Bersih (setelah dipotong ongkos giling {int(persentase_ongkos*100)}%):** {beras_bersih:,.1f} kg")
        st.write(f"• **Harga Jual Beras Final:** Rp {harga_beras_final:,} /kg (Dasar: Rp {harga_beras_dasar:,} + Sosoh: Rp {pengaruh_harga_beras:+,})")
        st.write(f"  👉 Pendapatan Beras: Rp {pendapatan_beras:,.0f}")
        st.write(f"• **Hasil Dedak ({persentase_dedak}%):** {estimasi_dedak_kg:,.1f} kg × Rp {harga_dedak:,} = Rp {pendapatan_dedak:,.0f}")
        st.write(f"• **Total Pengeluaran (Biaya Jemur Rp {total_biaya_jemur:,} + Transport Rp {biaya_transport_pribadi:,}):** Rp {total_pengeluaran_olah:,}")
        st.markdown(f"**Rumus Pendapatan Bersih:** (Pendapatan Beras + Pendapatan Dedak) - Total Pengeluaran = **Rp {total_pendapatan_olah:,.0f}**")
